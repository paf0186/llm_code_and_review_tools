...
class StagingManager:
    ...
    def __init__(self, staging_dir: Optional[Path] = None):
        ...
        Args:
            staging_dir: Directory to store staged operations.
                        Defaults to .gerrit-comments/staged/ in current working directory
        """
        if staging_dir is None:
            staging_dir = Path.cwd() / ".gerrit-comments" / "staged"
    ...
    def format_summary(self, staged: StagedPatch) -> str:
        ...
        lines.append("")
        lines.append(f"Push with: gerrit-comments push {staged.change_number}")
