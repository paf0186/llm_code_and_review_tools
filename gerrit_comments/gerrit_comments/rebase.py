...
class RebaseManager:
    ...
    def _complete_reintegration(
        ...
        lines = [
            ...
            f"  gerrit-comments review-series {session.series_url}",
        ]
    ...
    def _format_work_on_instructions(
        ...
        lines = []
        lines.append("=" * 70)
        lines.append(f"📝 Working on Patch {target_index + 1}/{len(series_patches)}")
        lines.append("=" * 70)
        ...
        try:
            ...
            if comments.threads:
                lines.append(f"📋 {len(comments.threads)} Unresolved Comment(s):")
                lines.append("-" * 70)
                ...
                lines.append("")
                lines.append("💬 To reply to comments:")
                lines.append("  gerrit-comments stage --done <index>     # Mark as done")
                lines.append("  gerrit-comments stage <index> \"message\"  # Reply with message")
            else:
        ...
        lines.append("3. gerrit-comments finish-patch  (rebases series, goes to next patch)")
        lines.append("")
        lines.append("When done with all patches: gerrit-comments end-session")
        lines.append("To discard all changes:     gerrit-comments abort-session")
        lines.append("")
        lines.append("=" * 70)
        ...
        try:
            ...
            if self._is_cherry_pick_in_progress():
                if self._has_unmerged_files():
                    ...
                    lines.append("  3. Run 'gerrit-comments finish-patch' again")
                    lines.append("")
            ...
            if descendants:
                ...
                if remaining:
                    lines.append(f"\n📦 Rebasing {len(remaining)} remaining patch(es)...")
                else:
                ...
                for desc in remaining:
                    ...
                    try:
                        ...
                        lines.append("  2. git add <resolved files>")
                        lines.append("  3. gerrit-comments finish-patch")
                        lines.append("")
                        lines.append("To skip this commit: git cherry-pick --skip")
                        lines.append("To abort: gerrit-comments abort-patch")
            ...
            if push_count > 0:
                if push_success:
                    lines.append(f"📤 Pushed {push_count} staged reply(ies) to Gerrit")
                else:
            ...
            lines.append("Next: gerrit-comments next-patch")
            lines.append("=" * 70)
    ...
    def get_status(self) -> tuple[bool, str]:
        ...
        lines = []
        lines.append("=" * 70)
        lines.append("📊 REBASE SESSION STATUS")
        lines.append("=" * 70)
        ...
        lines.append("  gerrit-comments finish-patch    # Complete the rebase")
        lines.append("  gerrit-comments abort-patch     # Abort and return to original state")
        lines.append("=" * 70)
...
def next_patch(with_comments: bool = False) -> tuple[bool, str]:
    ...
    session = manager.load_session()
    if not session:
        return False, "No active rebase session. Start one with: gerrit-comments work-on-patch <change> <url>"
