#!/usr/bin/env bash
# Bash tab completion for gerrit-comments
#
# To enable:
#   source /path/to/gerrit-comments-completion.bash
#
# Or add to ~/.bashrc:
#   source /path/to/gerrit-comments-completion.bash

_gerrit_comments_completions() {
    ...
    # Get the command (second word, after gerrit-comments)
    local cmd=""
    if (( cword >= 2 )); then
        cmd="${words[1]}"
    fi
...
complete -F _gerrit_comments_completions gerrit-comments

# Also provide completion for 'gc' alias if it exists
if type gc &>/dev/null; then
    complete -F _gerrit_comments_completions gc
fi
