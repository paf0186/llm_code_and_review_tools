...
def get_tool_description() -> ToolDescription:
    ...
    return ToolDescription(
        ...
        commands=[
            ...
            Command(
                name="worklogs",
                description="List worklogs (time tracking entries) for an issue",
                usage="jira worklogs <KEY> [--limit N] [--offset N]",
                arguments=[
                    Argument(name="key", description="Issue key or JIRA URL", required=True),
                    Argument(name="--limit", description="Maximum worklogs to return", type="integer", default=20),
                    Argument(name="--offset", description="Skip first N worklogs", type="integer", default=0),
                ],
                examples=["jira worklogs PROJ-123"],
                output_fields=["issue_key", "total", "worklogs[].id", "worklogs[].author", "worklogs[].time_spent", "worklogs[].started", "pagination"],
                next_actions=["worklog (to add)"],
            ),
            Command(
                name="worklog",
                description="Add a worklog entry to an issue",
                usage='jira worklog <KEY> <TIME_SPENT> [--comment TEXT]',
                arguments=[
                    Argument(name="key", description="Issue key or JIRA URL", required=True),
                    Argument(name="time_spent", description="Time in JIRA format (e.g., '2h 30m', '1d', '30m')", required=True),
                    Argument(name="--comment", description="Comment for the worklog entry"),
                ],
                examples=["jira worklog PROJ-123 2h", 'jira worklog PROJ-123 "1h 30m" --comment "Code review"'],
                output_fields=["issue_key", "worklog.id", "worklog.time_spent", "worklog.started"],
                next_actions=["worklogs", "get"],
            ),
            Command(
                name="watchers",
                description="List watchers for an issue",
                usage="jira watchers <KEY>",
                arguments=[
                    Argument(name="key", description="Issue key or JIRA URL", required=True),
                ],
                examples=["jira watchers PROJ-123"],
                output_fields=["issue_key", "count", "is_watching", "watchers[].name", "watchers[].display_name"],
                next_actions=["watch", "unwatch"],
            ),
            Command(
                name="watch",
                description="Add a watcher to an issue",
                usage="jira watch <KEY> [--user USERNAME]",
                arguments=[
                    Argument(name="key", description="Issue key or JIRA URL", required=True),
                    Argument(name="--user", description="Username to add (default: current user)"),
                ],
                examples=["jira watch PROJ-123", "jira watch PROJ-123 --user jdoe"],
                output_fields=["issue_key", "user", "action"],
                next_actions=["watchers"],
            ),
            Command(
                name="unwatch",
                description="Remove a watcher from an issue",
                usage="jira unwatch <KEY> [--user USERNAME]",
                arguments=[
                    Argument(name="key", description="Issue key or JIRA URL", required=True),
                    Argument(name="--user", description="Username to remove (default: current user)"),
                ],
                examples=["jira unwatch PROJ-123"],
                output_fields=["issue_key", "user", "action"],
                next_actions=["watchers"],
            ),
            Command(
                ...
                usage="jira attachment get <ATTACHMENT_ID>",
                arguments=[
                    Argument(name="attachment_id", description="Numeric attachment ID", required=True),
                ],
                examples=["jira attachment get 12345"],
                output_fields=["id", "filename", "size", "mime_type", "created", "author"],
                next_actions=["attachment content"],
            ),
            Command(
                name="config sample",
                description="Output a sample configuration file",
                usage="jira config sample",
                arguments=[],
                examples=["jira config sample"],
                output_fields=["sample_config", "default_path"],
            ),
            Command(
                name="config test",
