    ...
    if runs:
        ...
        for r in runs:
            ...
            if r.get("number") != build.get("number"):
                continue
            display = r.get("fullDisplayName", "")
            run_url = r.get("url", "")
            # Extract configuration from URL: arch=x86_64,build_type=client,...
            config_str = ""
            if "/job/" in run_url:
...
def jobs(
    ...
    Shows job name, build status, and health for all jobs
...
def builds(
    ...
    Examples:
      ...
      jenkins builds lustre-master --pretty
    """
    try:
        client = _make_client(url, user, token)
    ...
    if items:
        next_actions.append(
            f"jenkins build {job_name} {items[0]['number']}"
            f" -- details of most recent build"
        )
        failed = [b for b in items if b["result"] == "FAILURE"]
        if failed:
            next_actions.append(
                f"jenkins build {job_name} {failed[0]['number']}"
                f" -- details of most recent failure"
            )
...
def build(
    ...
    Examples:
      ...
      jenkins build lustre-master lastFailedBuild --pretty
    """
    try:
        client = _make_client(url, user, token)
    ...
    # Suggest run-console for failed matrix sub-builds
    runs = result.get("runs", [])
    failed_runs = [r for r in runs if r.get("result") == "FAILURE"]
    for fr in failed_runs[:2]:
        cfg = fr.get("config", "")
        if cfg:
            next_actions.append(
                f'jenkins run-console {job_name} {bnum} "{cfg}"'
                f" -- console for failed {cfg}"
            )

    # If it's a Gerrit-triggered build, suggest review lookup
    gerrit = result.get("gerrit")
    if gerrit and gerrit.get("change"):
        next_actions.append(
            f"jenkins review {gerrit['change']}"
            f" -- all builds for this Gerrit change"
        )

    # Suggest retrigger for failed builds
    if result.get("result") == "FAILURE":
        next_actions.append(
            f"jenkins retrigger {job_name} {bnum}"
            f" -- retrigger this failed build"
        )
...
def console(
    ...
    if grep_pattern:
        ...
        result = {
            ...
            "build": build_number,
            "total_lines": total_lines,
            "showing": f"last {len(selected)} lines",
            "lines": selected,
        }

    next_actions = [
        f"jenkins build {job_name} {build_number} -- build details",
    ]

    env = success_response(result, TOOL_NAME, "console", next_actions)
    _output(env, pretty)
...
def review(
    ...
    Examples:
      ...
      jenkins review 54225 --limit 30 --pretty
    """
    try:
        client = _make_client(url, user, token)
        if job:
            ...
            matches = client.find_review_builds(
                change_number, max_builds=limit
            )
    except requests.HTTPError as e:
    ...
    if items:
        ...
        next_actions.append(
            f"jenkins console {j} {b['number']}"
            f" -- console output of latest build"
        )
        failed = [b for b in items if b.get("result") == "FAILURE"]
        if failed:
            f = failed[0]
            fj = f.get("job", "lustre-reviews")
            next_actions.append(
                f"jenkins console {fj} {f['number']}"
                f" -- console output of failed build"
            )
...
def run_console(
    ...
    next_actions = [
        f"jenkins build {job_name} {build_number} -- full build details with all runs",
    ]

    env = success_response(result, TOOL_NAME, "run-console", next_actions)
    _output(env, pretty)
...
def abort(
    ...
    try:
        ...
        if not data.get("building", False):
            result: dict[str, Any] = {
                "job": job_name,
                "build": build_number,
                "message": f"Build {build_number} is not running (result: {data.get('result')})",
                "aborted": False,
            }
    ...
    env = success_response(result, TOOL_NAME, "abort", next_actions)
    _output(env, pretty)
...
def retrigger(
    ...
    env = success_response(result, TOOL_NAME, "retrigger", next_actions)
    _output(env, pretty)
...
def describe(command_name: str | None, pretty: bool) -> None:
    """Show machine-readable tool description.
    ...
    if command_name:
        ...
        if not matching:
            _error("NOT_FOUND", f"Command '{command_name}' not found", "describe", pretty)
            return
