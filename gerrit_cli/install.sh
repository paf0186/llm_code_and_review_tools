#!/bin/bash
#
# Installation script for gerrit CLI tool
# This script installs the gerrit CLI tool so it can be used as described in SKILLS.md
#
...
if [[ "$1" == "--help" ]] || [[ "$1" == "-h" ]]; then
    ...
    echo ""
    echo "Install the gerrit CLI tool"
    echo ""
    echo "Options:"
...
echo "================================================"
echo "Installing Gerrit CLI Tool"
echo "================================================"
echo ""

# Clean up build artifacts if requested or if they exist with wrong permissions
if [ "$CLEAN_INSTALL" = true ] || [ -d "$SCRIPT_DIR/gerrit_cli.egg-info" ]; then
    ...
    rm -rf "$SCRIPT_DIR/gerrit_cli.egg-info" 2>/dev/null || true
...
if [ "$(id -u)" != "0" ]; then
    ...
    CAN_WRITE=true
    EGG_INFO="$SCRIPT_DIR/gerrit_cli.egg-info"
    if [ -d "$EGG_INFO" ]; then
...
# Uninstall old gerrit-comments package if present
if $PYTHON_CMD -m pip show gerrit-comments &> /dev/null; then
    echo "Removing old gerrit-comments package..."
    $PYTHON_CMD -m pip uninstall gerrit-comments -y
fi

echo "Installing gerrit-cli package..."
cd "$INSTALL_DIR"
...
if [ -f "$SCRIPT_DIR/.env" ]; then
    ...
    if [ -f "$SCRIPT_DIR/.env" ]; then
        ...
        echo "  To set up credentials, create ~/.config/gerrit-cli/.env"
    fi
fi
...
echo ""
echo "Example usage:"
echo "  gerrit comments https://review.whamcloud.com/61965"
echo "  gerrit review-series https://review.whamcloud.com/61965"
echo ""

# Install tab completion
COMPLETION_SCRIPT="$SCRIPT_DIR/scripts/gerrit-completion.bash"
COMPLETION_LINE="source $COMPLETION_SCRIPT"
BASHRC="$HOME/.bashrc"

# Remove old completion line if present
if [ -f "$BASHRC" ]; then
    sed -i '/gerrit-comments-completion\.bash/d' "$BASHRC" 2>/dev/null || true
    sed -i '/# gerrit-comments tab completion/d' "$BASHRC" 2>/dev/null || true
fi
...
if [ -f "$COMPLETION_SCRIPT" ]; then
    if [ -f "$BASHRC" ]; then
        if grep -qF "$COMPLETION_SCRIPT" "$BASHRC" 2>/dev/null; then
            ...
            echo "" >> "$BASHRC"
            echo "# gerrit tab completion" >> "$BASHRC"
...
if command -v gerrit &> /dev/null; then
    ...
    echo -e "${YELLOW}⚠ Installation completed but 'gerrit' command not found in PATH${NC}"
